...
async def get_chat_history(chat, limit=25):
    ...
    async for message in client.iter_messages(chat, limit=limit):
        if message.text:  # Только текстовые сообщения
            ...
            if message.text.startswith("Тайлер:"):
                messages.append({
                    "role": "assistant",
                    "content": message.text[len("Тайлер:"):].strip(),
                    "sender_name": "Тайлер"
                })
            else:
                messages.append({
...
@client.on(events.NewMessage(pattern=r'^(?!Тайлер:)((\./)|(.*?[Тт]айлер.*?))'))  # ((.*?[Дд]жотаро.*?)|(.*?[Жж]отаро.*?)) - старые триггеры
...
async def handle_reply_to_bot(event):
    ...
    if not replied_msg or not replied_msg.text or not replied_msg.text.startswith("Тайлер:"):
        return
...
async def process_request(event):
    ...
    try:
        ...
        context_query = query
        if replied_msg and replied_msg.text:
            if replied_msg.text.startswith("Тайлер:"):
                # Если ответили на сообщение бота
                bot_response = replied_msg.text[len("Тайлер:"):].strip()
